...
def make_combined_alignment(alignment, aas):
  ...
  subprocess.call('mafft --keepgaponlysites --add aas.fasta ' + str(alignment) + ' > new_alignment.fasta', shell=True) 

  lines = list(SeqIO.parse(open('new_alignment.fasta', 'r'), 'fasta'))

  for line in lines:
    if line.id == 'ref_seq':
      protein_site = 0
      alignment_site = 0
      outfile = open('structure.map', 'w')
      outfile.write('aa,alignment,protein\n')
      for site in line.seq:
        alignment_site += 1
        if site != '-':
          protein_site += 1
          outfile.write(str(site) + ',' + str(alignment_site) + ',' + str(protein_site) + '\n')
        else:
          outfile.write(str(site) + ',' + str(alignment_site) + ',' + '-' + '\n')
      outfile.close()
      break
